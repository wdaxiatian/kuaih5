...
<div id="content">
    <div id="content-header">
        ...
        <h2>列表</h2>
        <a class="btn btn-info" href="<?php echo U('list/Getexcel16',array('star'=>$star,'end'=>$end))?>"  style="float:right; margin-right:100px;">导出当前表</a>
    </div>
        ...
        <div class="row-fluid">
            <div class="span12">
                <div class="widget-box">
                    <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
                        <h5>Data table</h5>
                        <form id="tform" action="<?php echo U('list/jsjj')?>" method="post">
			...
			<span style="float:left;  height: 35px;" >
				...
				<select name="status" id="status"style="margin-top:6px;width:120px;height:25px;font-size: 12px" >
                  ...
                  <option value="1" <?php if($status == 1):echo 'selected'?><?php endif;?>>邮轮大奖</option>
                  <option value="2" <?php if($status == 2):echo 'selected'?><?php endif;?>>金银币</option>
                  <option value="3" <?php if($status == 3):echo 'selected'?><?php endif;?>>未中奖</option>
                </select>
			</span>
...
<script>
    ...
    $("tbody").load("<?php echo U('list/listajax16')?>",{'page':page,'star':star,'end':end,'status':status},function() {
    });
    function ck(){
        var status =$("select option:selected").val();
